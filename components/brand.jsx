...
export function Icon({ color = 'black', size = 64, style = {}, ...rest }) {
...
export function Wordmark({ color = 'black', height = 32, style = {} }) {
  const textColor = {
    black: COLORS.ink900,
    white: COLORS.ink0,
...
export function WordmarkStacked({ color = 'black', size = 96, style = {} }) {
  const textColor = {
    black: COLORS.ink900,
    white: COLORS.ink0,
    indigo: COLORS.signal500,
...
export function L({ d, size = 24, stroke = 1.5, style = {}, fill = 'none', ...rest }) {
  return (
...
export function Ico({ name, size = 24, stroke = 1.5, style = {}, ...rest }) {
...
// Lane override — lets a host (the editor) re-lane any template without every
// template having to thread a prop through. Frame is the single root every
// template renders, so overriding here reaches all of them. null means "use the
// lane the template asked for", i.e. unchanged behaviour.
export const LaneOverrideContext = React.createContext(null);

export function LaneOverrideProvider({ lane, children }) {
  return (
    <LaneOverrideContext.Provider value={lane || null}>{children}</LaneOverrideContext.Provider>
  );
}

export function Frame({ lane = 'lane-mono', width, height, children, style = {}, className = '' }) {
  const override = React.useContext(LaneOverrideContext);
  const activeLane = override || lane;
  // Several templates pin their background inline, which would beat the lane
  // class. While a lane is overridden, drop that inline background so the
  // .lane-* rule in brand.css actually applies.
  const { background: _bg, ...noBg } = style;
  const finalStyle = override ? noBg : style;
  return (
    <div
      className={`tmpl ${activeLane} ${className}`}
      style={{
        width,
        height,
        position: 'relative',
...
export function BrandStamp({ lane = 'mono', size = 'sm', position = 'br' }) {
  const colors = {
    mono: 'black',
    dark: 'white',
    light: 'indigo',
...
export function DotGrid({ opacity = 0.05, spacing = 32, dotSize = 1, color = 'currentColor', style = {} }) {
  return (
    <div
      style={{
        position: 'absolute',
...
export function AbstractCompute({ style = {}, lane = 'editorial' }) {
